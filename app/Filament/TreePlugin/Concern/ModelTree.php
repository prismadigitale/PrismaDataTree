...
trait ModelTree
{
    ...
    public static function bootModelTree()
    {
        static::saving(function (Model $model) {
            // Use loose comparison for parent ID checks
            if (empty($model->{$model->determineParentColumnName()}) || $model->{$model->determineParentColumnName()} == static::defaultParentKey()) {
    ...
    public static function selectArray(?int $maxDepth = null): array
    {
        ...
        $result[static::defaultParentKey()] = __('filament.tree-plugin.filament-tree.root');
